      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <Card>
          ...
          {outlets.map((outlet) => (
            <div key={outlet} style={{ marginBottom: 12 }}>
              ...
              {(staff[outlet] || []).length === 0 && (
                <div style={{ fontSize: 12, color: C.muted, padding: '4px 0' }}>No staff assigned</div>
              )}
            </div>
          ))}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
            ...
            <Btn small onClick={handleAddStaff}>+ Add Staff</Btn>
          </div>
        </Card>

        {/* Save */}
        <Btn full onClick={handleSave}>Save Settings</Btn>
      </div>
    </Sheet>
  )
}
